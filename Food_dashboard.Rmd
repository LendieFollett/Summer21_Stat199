...
### Predictions
```{r, echo=FALSE}
temp1 <- shp_file %>% as.data.frame%>%dplyr::select(c("GEOID","COUNTYNAME","fsecurity_predictions", "female", "hispanic",
                                  "black", "kids", "elderly", "education", "employed",
                                  "married", "disability"))%>%
                  rename( 
                         `Predicted Average` = fsecurity_predictions)
...
renderDataTable({
rbind(temp1[which(shp_file$COUNTYNAME == input$State),],
      temp1[which(shp_file$COUNTYNAME != input$State),])
...
### Predictions
```{r, echo=FALSE}
temp2 <- shp_file2 %>%as.data.frame%>% dplyr::select(c("GEOID","COUNTYNAME", "fexpend_predictions","female", "avg_hhsize", "hispanic",
                                  "black", "kids", "elderly", "education", "employed",
                                  "married", "disability"))%>%
                  rename(
...
renderDataTable({
rbind(temp2[which(shp_file2$COUNTYNAME == input$State2),],
      temp2[which(shp_file2$COUNTYNAME != input$State2),])
