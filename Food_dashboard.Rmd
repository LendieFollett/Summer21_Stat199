...
a1 <- ggplot(data = cps_fsecurity, aes (x = fsecurity))+geom_bar() + geom_text(stat = 'count',aes(label=..count..), vjust = -1) + labs(x = "Food Insecurity Level", y = "Number of Households")

a2 <- ggplot(data = cps_fexpend, aes (x = fexpend, fill = fexpend))+geom_histogram(binwidth = 5) + labs(x = "Food Expense In USD", y = "Number of Households") + scale_x_continuous(labels=scales::dollar_format()) + scale_fill_brewer(palette = "BuGn")
