...
source("Ryan_Code/fexpend_analysis.R")
source("Ryan_Code/fsecurity_analysis.R")
...
shp_file <- block_groups(state = State_stuff$state.abb[State_stuff$state.name == input$State])

shp_join <- left_join(shp_file, acs, by="GEOID" ) %>%
  rmapshaper::ms_simplify( keep = 0.01, keep_shapes = TRUE)

shp_join$COUNTYNAME <- convert_fips_to_names(shp_join$COUNTYFP, states = rep("State", nrow(shp_join)), geo_header="COUNTY")

shp_join$labels <- paste0(
  "<strong> County: </strong> ",
  shp_join$COUNTYNAME, "<br/> ",
  "<strong> GEOID: </strong> ",
  shp_join$GEOID, "<br/> ",
  "<strong> Food Expenditure: </strong> $",
  shp_join$fexpend_predictions, "<br/> ",
  "<strong> Food Insecurity: </strong> ",
  shp_join$fsecurity_predictions, "<br/> "
) %>%
  lapply(htmltools::HTML)

#for color scale
limit <- c(min(acs$fexpend_predictions,na.rm=TRUE), max(acs$fexpend_predictions,na.rm=TRUE))
limit_bin <- c(min(acs$fsecurity_predictions,na.rm=TRUE), max(acs$fsecurity_predictions,na.rm=TRUE))
pal_bin <- colorBin(
  palette = "YlOrRd", domain = ia_shp_join$fsecurity_predictions,
  bins = seq(0, max(ia_shp_join$fsecurity_predictions, na.rm = TRUE) + .01, by = .05)
)
pal <- colorBin(
  palette = "YlOrRd", domain = ia_shp_join$fexpend_predictions,
  reverse=TRUE,
  bins = seq(floor(min(ia_shp_join$fexpend_predictions, na.rm = TRUE)), floor(max(ia_shp_join$fexpend_predictions, na.rm = TRUE)) + .1, by = 5)
)
```
...
cnty_shp_join <- reactive({
  subset(shp_join, STATENAME == input$State) 
...
final_fsecurity$GEOID = as.character(paste0(final_fsecurity$GEOID, substr(final_fsecurity, 13, 13)))

final_fexpend$GEOID = as.character(paste0(final_fexpend$GEOID, substr(final_fexpend, 13, 13)))
...
shp_file <- block_groups(state = State_stuff$state.abb[State_stuff$state.name == "Iowa"]) %>%
...
pal_bin <- colorBin(
  palette = "YlOrRd", domain = final_combined$fsecurity_predictions,
  bins = seq(0, max(final_combined$fsecurity_predictions, na.rm = TRUE) + .01, by = .05)
)
pal <- colorBin(
  palette = "YlOrRd", domain = final_combined$fexpend_predictions,
  reverse=TRUE,
  bins = seq(floor(min(final_combined$fexpend_predictions, na.rm = TRUE)), floor(max(final_combined$fexpend_predictions, na.rm = TRUE)) + .1, by = 5)
)
