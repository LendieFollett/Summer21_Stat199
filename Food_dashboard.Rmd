...
selectInput("State", label = "Select State:", choices = unique(state.abb)[order(unique(state.abb))], selected = NULL)

shp_file <- block_groups(state = state.abb["State"])

shp_join <- left_join(shp_file, acs, by="GEOID" ) %>%
  rmapshaper::ms_simplify( keep = 0.01, keep_shapes = TRUE)

shp_join$COUNTYNAME <- convert_fips_to_names(ia_shp_join$COUNTYFP, states = rep("State", nrow(shp_join)), geo_header="COUNTY")
...
shp_join$labels <- paste0(
  ...
  shp_join$lasso_pred, "<br/> ",
  "<strong> Food Insecurity: </strong> ",
  shp_join$lasso_bin_pred, "<br/> "
) %>%
