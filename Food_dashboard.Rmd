...
Important Variables - Food Security {data-navmenu="Important Variables"}
===
...
Important Variables - Food Expenditure {data-navmenu="Important Variables"}
===
Column {.tabset}
---
...
### Descriptive Model - Food Security
...
h1 <- fsecurity.glm2
h2 <- beta_hat <- coef(fsecurity.glm2)
h3 <- exp(beta_hat)
h4 <- exp(confint(fsecurity.glm2))
...
### Descriptive Model - Food Expenditure
```{r, fig.width = 15, fig.height = 12}
...
fexpend.glm3 <- glm(fexpend_0 ~ hhsize + elderly + employed + disability + education + urban_c, data = cps_fexpend, family = binomial(link = "log"))
