...
shp_file <- reactive({ block_groups(state = State_stuff$state.abb[State_stuff$state.name == "Iowa"]) %>%
...
renderLeaflet({
  leaflet(shp_file(),height=500, width=1000) %>%
    ...
    addPolygons(
      fillColor = ~ pal(fsecurity_predictions),
      stroke=FALSE,
      color = "white",
      fillOpacity = 0.6,
      label = ~labels,
    ...
    leaflet::addLegend(
      pal = pal, values = ~fsecurity_predictions,
...
shp_file2 <- reactive({ block_groups(state = State_stuff$state.abb[State_stuff$state.name == "Iowa"]) %>%
    left_join(final_combined, by="GEOID" ) %>%
    rmapshaper::ms_simplify( keep = 0.01, keep_shapes = TRUE) %>%
    #     mutate(COUNTYNAME = convert_fips_to_names(COUNTYFP, states = rep("State", nrow(shp_join)), geo_header="COUNTY"))
    mutate(labels = paste0(
